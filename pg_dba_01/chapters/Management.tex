...
file, just one page, called pg\_control. This file is vital for the cluster's activity
...
the write ahead logs, \index{wal files} or WAL files. Each segment is by default 16 MB and is used
to store the records pages modified during the cluster's activity. Writing the changes first on
this durable storage ensures the cluster's recovery capability. In the case of a crash WAL
records are replayed during the initial startup's crash recovery. The last checkpoint location
is read from control file and is from where the replay starts. Because this directory is heavily
written, putting it on a dedicated device improves the performance. 
