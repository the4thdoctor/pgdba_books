...
The optional ANALYZE clause gather the runtime statistics on processed table.\newline
...
The maintenance\_work\_mem is used by VACUUM to store an array of TCID pointers to the dead tuples and 
used for the index cleanup. With a small maintenance\_work\_mem the memory runs out of space. When this 
happens the table scan pauses and the index is scanned searching for the tuples stored into the array. When 
the index scan is complete the array cleared and the table's scan resumes. Increasing the 
maintenance\_work\_mem to 2 
GB\footnote{In order to have the table in the same conditions the table was cleared with a VACUUM FULL 
and bloated with a new update.} the index scan is executed in one single run 
...
Removing the indices, in our case the primary key, improves the VACUUM performance 
because the maintenance\_work\_mem is not used.\newline
